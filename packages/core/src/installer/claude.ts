...
export const installToClaude = async ({
  ...
  if (!existsSync(CLAUDE_CONFIG_PATH)) {
    throw new AppError(ErrorCode.NOT_FOUND, `Claude config file not found at: ${CLAUDE_CONFIG_PATH}`);
  }
  ...
  const updatedConfig = {
    ...
    mcpServers: {
      ...
      [`${CLAUDE_CONFIG_KEY_PREFIX}__${name}`]: {
        args: ["run", path.resolve(__dirname, "../../bin/cli.ts"), "start", name, "--transport", "stdio"],
        command: "bun",
      },
    },
  };
  ...
  await fs.writeFile(CLAUDE_CONFIG_PATH, JSON.stringify(updatedConfig, null, 2));
...
export const uninstallFromClaude = async ({
  ...
  logger.info(`uninstalling from Claude configuration in ${CLAUDE_CONFIG_PATH}`);
  ...
  if (!claudeConfig.mcpServers[serverKey]) {
    logger.info(`Server "${name}" not found in Claude config, nothing to uninstall`);
    return;
  }
