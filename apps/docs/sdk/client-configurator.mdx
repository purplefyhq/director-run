...
The Client Configurator is a library that helps manage MCP Client configuration (checking client status, adding / removing servers, etc). Currently it supports Claude, Cursor & VSCode. The source code is available in [packages/client-configurator](https://github.com/theworkingcompany/director/tree/main/packages/client-configurator).
