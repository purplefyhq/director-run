...
import { Badge, BadgeGroup, BadgeLabel } from "@director.run/design/ui/badge";
...
// Returns true if all schemas in the union are simple types (not object, array, or another union)
const isSimpleUnion = (schemas: JsonSchemaProperty[]): boolean => {
...
const UnionTypeRenderer: React.FC<UnionTypeRendererProps> = ({
  ...
  if (isSimpleUnion(schemas)) {
    const types = schemas.map((schema) =>
      formatType(schema.type, schema.format),
    );
    return (
      <div className="flex flex-row flex-wrap gap-x-2 gap-y-3.5 rounded-md bg-surface-subtle p-4">
        <Badge variant="orange">
          <BadgeLabel uppercase>{unionType}</BadgeLabel>
        </Badge>
        <span className="grow text-gray-600 text-sm leading-6">
  ...
  return (
    <div className="mt-4">
      ...
      <Accordion
        className="flex flex-col border-accent border-y-[0.5px]"
        type="multiple"
      >
...
function PropertyDefault({ property }: { property: JsonSchemaProperty }) {
  ...
  return (
    <div className="flex flex-row gap-x-2">
      <span className="font-medium text-content-secondary text-sm leading-6 tracking-wide">
        Default:
      </span>
          ...
          {!hideHeader && (
            ...
            >
              <AccordionTrigger asChild>
                <Button variant="tertiary" size="sm" className="!size-6">
                  <CaretRightIcon
                    weight="bold"
                    className="size-4 transition-transform duration-200 [[data-state=open]_&]:rotate-90"
                  />
...
function PropertyItem({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex flex-col border-b-hairline py-3 outline-none last:border-b-0",
        className,
      )}
      {...props}
    >
      ...
      <div className="shrink-0">
        {children ?? (
          <div className="flex size-6 items-center justify-center">
            <CircleIcon
              weight="fill"
              className="size-2 text-content-secondary"
            />
          </div>
        )}
      </div>
      <div>
        <span className="inline-flex rounded-md bg-surface-neutral px-1.5 font-medium font-mono text-sm leading-6 ">
          {name}
        </span>
      </div>
      <BadgeGroup>
        ...
        <Badge
          variant={
            ["oneOf", "anyOf", "allOf"].includes(type) ? "orange" : "secondary"
          }
        >
          <BadgeLabel uppercase>{type}</BadgeLabel>
        </Badge>
...
function PropertyContent({
  ...
  return (
    <div
      className={cn("ml-8 flex flex-col gap-y-3 pt-2", className)}
      {...props}
    >
      {children}
    </div>
...
export const JSONSchema: React.FC<JSONSchemaProps> = ({ schema }) => {
  ...
  if (!schema.properties || Object.keys(schema.properties).length === 0) {
    return (
      <NonIdealState>
        <NonIdealStateTitle>No parameters defined</NonIdealStateTitle>
        <NonIdealStateDescription>
  ...
  return (
    <Accordion
      className="flex flex-col border-accent border-t-hairline border-b-hairline"
      type="multiple"
    >
      {Object.entries(schema.properties).map(
